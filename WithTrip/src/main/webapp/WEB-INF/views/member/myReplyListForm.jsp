...
<style>
		...
		/* 댓글 삭제 모달 END */
		/* 댓글 삭제 버튼(모달아님 리스트에 있는 버튼) */
		#deleteBtn{background-color: lightgrey; border: none; color:white; border-radius: 5px;}
</style>
</head>
  <body id="page-top">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top" id="sideNav" style="position:absolute;">
      ...
      <div class="collapse navbar-collapse"  id="navbarSupportedContent">
        <ul class="navbar-nav">
			...
			<li class="nav-item">
			  <a class="nav-link js-scroll-trigger" onclick="location.href='myReply.me';" style="color: white;">작성한 용품 후기</a>
			</li>
			<li class="nav-item">
...
<div class="container rounded mt-5 bg-white p-md-5">
    <div class="main">
	<!-- 페이지 타이틀 -->
   	<input type="checkbox" checked="checked" id="pageTitle"><label class="pageLabel">작성한 용품 후기</label>
   	<div class="tableline">
    <div class="table-responsive">
        <table class="table">
            ...
            <c:if test="${ empty list }">
            	<tbody>
                   <tr>
                     <td colspan="5" style="align-items: center;">작성한 용품 후기가 없습니다.</td>
                   </tr>
               </tbody>
            </c:if>
            <c:if test="${ !empty list }">
	            <c:forEach var="rv" items="${ list }" varStatus="status">
		            <tbody>
		                <tr class="bg-blue">
			                <td><input type="hidden" name="gId" value="${ rv.goodsId }">
			                ...
			                <td>
			                	<a href="javascript:userPage()" style="color: black;">${ rv.goods_name }</a>
			                </td>
				      		<td>${ rv.replyContent }</td>
				      		<td>${ rv.createDate }</td>      
				      		<td>
				      			<button type="button" value="${ rv.replyId }" id="deleteBtn" class="deleteBtn">삭제</button>
				      		</td>
		                </tr>
		            </tbody>
		        </c:forEach>
		...
		<!-- 용품 후기 댓글 삭제 모달 START -->
		    <div class="deleteModal">
			    <div  class="deleteModal_body">
					<div><hr class="modal_line"></div>
		            	<div class="modal_content">해당 후기를 삭제하시겠습니까?</div>
		            <div><hr class="modal_line"></div>
               ...
               <!-- main템플릿 페이지네이션 -->
                 ...
                 <div class="col text-center">
                   <div class="block-27">
                     ...
                     <c:if test="${ pi.currentPage > 1 }">
                     	<c:url var="before" value="myReply.me">
                     ...
                     <c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
                       ...
                       <c:if test="${ p ne pi.currentPage }">
                       		<c:url var="pagination" value="myReply.me">
                       ...
                       <c:if test="${ pi.currentPage < pi.maxPage }">
                       		<c:url var="after" value="myReply.me">
                       			<c:param name="page" value="${pi.currentPage + 1 }"/>
                       		</c:url>
                       		<li>
...
<script>
 ...
 // 프로필 사진 클릭 시 사진 모달 띄우기
    $(function(){
 	...
 	// 댓글 내용 클릭 시 해당 용품 상세보기로 이동
 	$('.bg-blue').find('td').mouseenter(function(){
		gId = $(this).parent().children().find('input').val();
		console.log($('#rId').val());
 	})
 	...
 	function userPage(){
    	...
    	obj.setAttribute("name", "gId");
    	obj.setAttribute("value", gId);
    	
    	form.appendChild(obj);
    	form.setAttribute("method", "post");
    	form.setAttribute("action", 'gdetail.go?gId=' + gId);
    ...
    deleteModal.addEventListener('click', function(event){
	 ...
	 // 내가 쓴 용품 후기 댓글 삭제
	 function clickDel(){
		location.href="myRdelete.go?rId=" + rId;
	 }
</script>
</body>
</html>